...
describe('PUT /booking/:bookingId', () => {
  ...
  describe('when token is valid', () => {
    ...
    describe("when body is valid", () => {
      ...
      it('Should response with status 403 if given bookingId doesnt exist', async () => {
        const user = await createUser();
        const token = await generateValidToken(user);
        const enrollment = await createEnrollmentWithAddress(user);
        const ticketType = await createTicketTypeWithHotel();
      ...
      it('Should response with status 403 if given bookingId doesnt belong to user', async () => {
        const user = await createUser();
        ...
        const room = await createRoomWithHotelId(hotel.id);
        const body = { roomId: room.id };
        const booking = await createBooking(userTest.id, room.id)
        const response = await server.put(`/booking/${booking.id}`).set('Authorization', `Bearer ${token}`).send(body);

        expect(response.status).toBe(httpStatus.FORBIDDEN);
      });

      it('Should response with status 200 and roomId', async () => {
        const user = await createUser();
        const token = await generateValidToken(user);
        const enrollment = await createEnrollmentWithAddress(user);
        const ticketType = await createTicketTypeWithHotel();
